...
> ⚠️ **Data Notice**: The flood prevention and water conservancy source documents (under `data/防洪预案/`) are **internal confidential files** and are **not included** in this public repository. To reproduce Phase 2 & 3 experiments, you need to provide your own flood prevention plan documents in PDF format and place them in `data/防洪预案/`.
...
- Test questions: n=30 (3 types, 10 per type)

**⚠️ Data Note**: This section presents **ablation study data** (generated by phase3_reranker_ablation.py).
- The Baseline and CR accuracy values in the ablation study use slightly different evaluation criteria
- The complete Phase 3 enhanced experiment (without Reranker) uses standard evaluation metrics
- For rigorous comparison of Baseline vs CR without Reranker, please refer to the paper's main results

```bash
python scripts/phase3_reranker_ablation.py
```

#### 2×2 Ablation Results Table (Reranker Ablation Design)
...
#### Average Score Comparison (Ablation Study)
...
*Note: Ablation study evaluation criteria differ from the enhanced experiment due to different data sources.

#### Phase 3 Enhanced Experiment Complete Data (Without Reranker)

For accurate Baseline vs CR comparison, refer to the standard enhanced experiment results:

| Method | Accuracy | Correct Count | Avg Score |
|--------|----------|---------------|-----------|
| Baseline | **76.7%** | 23/30 | 0.5145 |
| **CR** | **80.0%** | 24/30 | 0.5188 |

This demonstrates CR's **3.3% improvement** over Baseline (76.7% → 80.0%), with uneven distribution:
- Type A (Numeric queries): 80% → 90% (+10%)
- Type B (Entity queries): 70% → 70% (no change)
- Type C (Process queries): 80% → 80% (no change)

#### Statistical Tests

| Comparison | Test Method | t-value | p-value | Conclusion |
|------------|-------------|---------|---------|------------|
| Baseline → Baseline+RR (ablation) | Paired t-test | -17.14 | <0.001 | ✅ Reranker significantly improves |
| CR → CR+RR (ablation) | Paired t-test | -17.13 | <0.001 | ✅ Reranker significantly improves |
