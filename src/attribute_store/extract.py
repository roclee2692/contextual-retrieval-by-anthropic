...
# 条款/表号简单识别
# 避免误把数值(如113.5)当作条款号：优先匹配多级条款号 3.2.1
...
# 字段正则模板（至少 10 条）
# 注：对表格字段，允许字段名和数值之间有逗号/顿号/空格/换行等分隔
FIELD_PATTERNS: List[Tuple[str, re.Pattern]] = [
    ...
    ("总库容", re.compile(r"总\s*库\s*容[:：，、；\s]*(\d+(?:\.\d+)?)\s*(万m³|亿m³|万m3|亿m3|万方|亿方|m³|m3|万立方米|亿立方米|立方米)")),
    ("调洪库容", re.compile(r"调\s*洪\s*库\s*容[:：，、；\s]*(\d+(?:\.\d+)?)\s*(万m³|亿m³|万m3|亿m3|万方|亿方|m³|m3|万立方米|亿立方米|立方米)")),
    ("兴利库容", re.compile(r"兴\s*利\s*库\s*容[:：，、；\s]*(\d+(?:\.\d+)?)\s*(万m³|亿m³|万m3|亿m3|万方|亿方|m³|m3|万立方米|亿立方米|立方米)")),
    ("防洪库容", re.compile(r"防\s*洪\s*库\s*容[:：，、；\s]*(\d+(?:\.\d+)?)\s*(万m³|亿m³|万m3|亿m3|万方|亿方|m³|m3|万立方米|亿立方米|立方米)")),
    ("死库容", re.compile(r"死\s*库\s*容[:：，、；\s]*(\d+(?:\.\d+)?)\s*(万m³|亿m³|万m3|亿m3|万方|亿方|m³|m3|万立方米|亿立方米|立方米)")),
    ("坝顶高程", re.compile(r"坝\s*顶\s*高\s*程[:：，、；\s]*(\d+(?:\.\d+)?)\s*(米|m)")),
    ("坝高", re.compile(r"坝\s*高[:：，、；\s]*(\d+(?:\.\d+)?)\s*(米|m)")),
    ("坝长", re.compile(r"坝\s*长[:：，、；\s]*(\d+(?:\.\d+)?)\s*(米|m)")),
    ("坝顶宽", re.compile(r"坝\s*顶\s*宽[度]?[:：，、；\s]*(\d+(?:\.\d+)?)\s*(米|m)")),
    ("流域面积", re.compile(r"流\s*域\s*面\s*积[:：，、；\s]*(\d+(?:\.\d+)?)\s*(km²|平方公里|km2)")),
    ("设计流量", re.compile(r"设计流量[:：，、；\s]*(\d+(?:\.\d+)?)\s*(m³/s|m3/s|立方米每秒|立方米/秒)")),
    ("泄洪流量", re.compile(r"泄洪流量[:：，、；\s]*(\d+(?:\.\d+)?)\s*(m³/s|m3/s|立方米每秒|立方米/秒)")),
    ("最大泄量", re.compile(r"最大泄[洪]?流?量[:：，、；\s]*(\d+(?:\.\d+)?)\s*(m³/s|m3/s|立方米每秒|立方米/秒)")),
    ("溢洪道设计流量", re.compile(r"溢\s*洪\s*道\s*设计流量[:：，、；\s]*(\d+(?:\.\d+)?)\s*(m³/s|m3/s|立方米每秒|立方米/秒)")),
    ("溢洪道最大泄量", re.compile(r"溢\s*洪\s*道\s*最大泄[洪]?流?量[:：，、；\s]*(\d+(?:\.\d+)?)\s*(m³/s|m3/s|立方米每秒|立方米/秒)")),
    ("降雨量", re.compile(r"(日降雨量|降雨量|雨量)[:：，、；\s]*(\d+(?:\.\d+)?)\s*(mm|毫米)")),
    ("时限", re.compile(r"(\d+)\s*(小时|h|分钟|min|天)\s*(内|以内|之内)")),
]
...
def extract_facts(text: str, source_doc: str, source_page: str) -> List[Dict[str, Any]]:
    ...
    for field, pat in FIELD_PATTERNS:
        for m in pat.finditer(text):
            ...
            # 结构区分：若上下文包含“溢洪道”，则归入溢洪道字段
            if field == "设计流量" and "溢洪道" in ctx:
                field = "溢洪道设计流量"
            elif field == "最大泄量" and "溢洪道" in ctx:
                field = "溢洪道最大泄量"
